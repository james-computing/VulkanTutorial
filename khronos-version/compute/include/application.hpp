...
private:
    ...
    std::vector<vk::raii::DeviceMemory> computeUniformBuffersMemories;
    std::vector<void*> computeUniformBuffersMapped;

    size_t const PARTICLE_COUNT {1 << 11};
    size_t const NUM_THREADS {1 << 8};
    vk::DeviceSize const shaderStorageBufferSize {PARTICLE_COUNT * sizeof(Particle)};
